const TestimonialCard = ({ name, action, quote, avatar }) => {
  return (
    <div
      className="bg-[#FDF9FC] rounded-xl p-10 flex flex-col gap-6 flex-1 text-right"
      dir="rtl"
    >
      ...
      <p className="text-gray-700 leading-relaxed font-medium w-2xl">"{quote}"</p>
    </div>
  );
};
...
const StatCard = ({ main, sub, showCurrency }) => {
  return (
    <div className="bg-primary rounded-3xl px-10 py-5 flex flex-col items-end text-white min-w-[280px] h-full shadow-lg">
      <div className="flex items-baseline gap-2">
        {showCurrency && <span className="text-2xl font-bold">₪</span>}
        <h3 className="text-5xl font-bold">{main}</h3>
      </div>
      <p className="text-sm font-bold opacity-90 mt-2">{sub}</p>
    </div>
  );
};
...
const Testimonials = () => {
  ...
  return (
    <section className="mx-auto px-10 flex flex-col gap-8">
